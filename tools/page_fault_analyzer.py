...
def main():
    ...
    print(f"开始监控{'PID ' + str(args.pid) if args.pid else '所有进程'} 的页面错误情况...")
    ...
    for i, (stack_id, pid, comm, major, minor, total) in enumerate(sorted_stacks[:args.top]):
        print(f"\n#{i+1} 进程: {comm} (PID: {pid})")
        print(f"主要页面错误: {major}, 次要页面错误: {minor}, 总计: {total}")
        print("\n调用堆栈:")
