...
def print_conn_event(cpu, data, size):
    ...
    # 打印事件
    print(f"[{datetime.fromtimestamp(event.ts_us / 1e6).strftime('%H:%M:%S.%f')}] "
...
def main():
    ...
    print(f"开始监控{'PID ' + str(args.pid) if args.pid else '所有进程'} 的网络活动...")
    ...
    # 打印进程网络统计信息
    print("\n===== 进程网络统计 =====")
    print(f"{'PID':<7} {'进程名':<16} {'接收(MB)':>12} {'发送(MB)':>12} {'总流量(MB)':>12} {'数据包':>10} {'重传':>8} {'重传率%':>10}")
    print("-" * 90)
